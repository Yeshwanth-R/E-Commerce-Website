...
const ProductForm = () => {
    ...
    const addProduct = (e) => {
        ...
        const myPromise = async () => {
            let res = await fetch(params.id ? `/api/findProducts/${params.id[0]}` : "/api/productsAdd", {
                method: "POST",
                headers: {
        ...
        myPromise()
            .then((data) => {
                toast.success(params.id ? `${data.Name} Edited successfully` : `${data.Name} Added Succesfully`, {
